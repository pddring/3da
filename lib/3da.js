var Assessment = function(subject) {
				...
				case 'btn_dlg_ok':
					...
					for(var i = 0; i < lines.length; i++) {
						...
						if(line.length > 0) {
							...
							c.topicIndex = i;
							c.criteriaIndex = topic.criteria.length - 1;
						}
					}
					a.updateTopics();
	...
	this.updateCharts = function(criteria) {
		criteria.data.setValue(0, 1, criteria.understanding);
		criteria.data.setValue(1, 1, criteria.independence);
		criteria.chart.draw(criteria.data, criteria.options);
	}
	...
	this.updateTopics = function() {
		...
		for(var i = 0 ; i < a.topics.length; i++) {
			...
			html += '<div id="chart_topic_summary_' + i + '"></div>'
			for(var j = 0; j < a.topics[i].criteria.length; j++) {
				...
				html += '<div class="criteria" id="criteria_' + i + '_' + j + '"><h3>' + a.descriptions.difficulty[a.topics[i].criteria[j].difficulty] + '</h3>' + a.topics[i].criteria[j].name + '<div><div class="criteria_indicator" id="criteria_indicator_' + i + '_' + j + '"></div></div><a data-role="button" data-icon="edit" data-role="button" class="btn_grade" id="btn_grade_' + i + '_' + j + '" data-inline="true">Grade</a></div></div>';
			}
			html += '</div>';
		}
		...
		for(var i = 0 ; i < a.topics.length; i++) {
			var data = google.visualization.arrayToDataTable([
				['Performace', 'My progress', 'Class average', 'National Standard'],
				['Independence', 5, 4, 4],
				['Understanding', 3, 3.5, 2.8],
				['Difficulty', 3, 4, 2.7]
			]);
			var options = {
			  chart: {
				title: 'Topic summary',
				subtitle: a.topics[i].name,
			  }
			};
			var chart = new google.charts.Bar(document.getElementById('chart_topic_summary_' + i));
			...
			for(var j = 0; j < a.topics[i].criteria.length; j++) {
				var data=google.visualization.arrayToDataTable([
					['Label',          		'Value'],
					['Understanding',  		a.topics[i].criteria[j].understanding],
					['Independence',        a.topics[i].criteria[j].independence],
				  ]);
				  
				var options = {
				  width: 400, height: 100,
				  redFrom: 0, redTo: 0.5,
				  greenFrom:4.5, greenTo:5,
				  max:5,
				};

				a.topics[i].criteria[j].data = data;
				a.topics[i].criteria[j].options = options;  
				a.topics[i].criteria[j].chart = new google.visualization.Gauge(document.getElementById('criteria_indicator_' + i + '_' + j));
				
				a.topics[i].criteria[j].chart.draw(data, options);
			}
		}
